...
- **Match Data & Editing — Annotation templates (Apply previous annotation)** — the Match Journal head's "Apply previous" button copies the chronologically previous annotated match's group members + tag set into the open match's draft (replacing it), then waits: Confirm persists through the normal `PUT /annotation` path, Undo restores the prior draft. Note, replay code, and leaver are never copied — they're per-match by nature. Committing any other journal field while the apply is pending persists the copied values too (every commit writes all drafts) and counts as implicit confirmation. Source lookup is the pure `previousAnnotatedRecord` helper over the narrowed set; e2e in `frontend/tests/e2e/annotation-apply-previous.spec.ts`.
