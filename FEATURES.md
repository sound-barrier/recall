...
- **Background watcher status indicator** — small dot in the masthead showing "watching · N new" so the state is visible from every tab. Two states: idle (dim green) and processing (pulsing accent); tooltip surfaces the most recent activity timestamp. *Partially shipped:* `MastheadParseChip` surfaces an in-flight parse from every tab; the idle "watching" dot remains.
...
- **Vacuum scheduler** — auto-run `PRAGMA optimize` after every N parse cycles, with a manual "Vacuum now" button in Advanced. Keeps the DB compact without requiring user intervention. *Partially shipped:* the manual Compact/optimize buttons live in Settings → Advanced → Database health; the auto-scheduler remains.
