...
- **Match Data & Editing — Match notes search** — free-text input in the FilterRail's filter-bar (between the multi-select grid and the date-range row) substring-matches against `annotation.note`. Case-insensitive, whitespace-trimmed; records without a note drop out while the query is non-empty. State lives in `useMatchFilters.noteSearch` and participates in `anyFilter` / `activeFilterCount` so the active-filter pills and "Clear Filters" surface it consistently. Inline × clear control; `/` keyboard shortcut focuses the input from anywhere in the app (see Keyboard shortcuts entry below). Expanded match cards render the saved note as a click-to-edit preview that wraps every active hit in `<mark class="note-hit">` — accent-soft fill, sharp under-rule, `target-acquired` HUD animation. Clicking the preview promotes it to the existing textarea at the click position (caret-from-point lookup); blur reverts to the preview with the (possibly edited) text re-highlighted against the live query. Filter math in `useMatchFilters.ts`; input + clear chrome in `FilterRail.vue`; preview swap + mark rendering in `MatchCardExpanded.vue`; coverage in `frontend/tests/e2e/match-notes-search.spec.ts` (substring narrowing, case-insensitivity, hit count, click-to-edit promote, blur-restores-preview).
