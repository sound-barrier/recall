...
- **Match exclusion reasons** — annotation field for *why* a match is being de-emphasized (placement / MMR adjustment / internet outage). Drives a filter that hides them from win-rate calculations without removing them from the list, complementing the existing leaver-handling control and the leaver / thrower side facets.
...
- **Ingest & OCR — Failed-file triage + diagnostic bundle** — screenshots whose OCR attempt fails outright are recorded in a `failed_files` ledger and surfaced in a new Unknown-tab "Failed to read" section (error verbatim, attempt tally, hover thumbnail, two-click Delete forever). Failures are retried every run by design — the ledger is visibility, not a skip list. A one-click "Save diagnostic bundle" builds a shareable zip (failing images + logs + version/environment manifest, no settings/DB) via native save dialog (desktop) or browser download (server). Root cause of the v0.25.0 "6 remaining forever" report.
