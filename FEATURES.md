...
- **Match exclusion reasons** — annotation field for *why* a match is being de-emphasised (placement / MMR adjustment / internet outage). Drives a filter that hides them from win-rate calculations without removing them from the list, complementing the existing leaver-handling control and the leaver / thrower side facets.
