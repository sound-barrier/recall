...
// minute correlate.MergeWindow). Same map + same hero on both. DIFFERENT EAD
...
// conflicts.
//
// PR #106 surfaces this as ambiguous via the new correlate.TieToleranceWindow
...
func TestCorrelation_Stress_TimestampWindowEdge(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		...
		// the 5 s correlate.TieToleranceWindow → ambiguous sentinel.
		specs = append(specs, matchSpec{
...
//
// Before PR #105, correlate.RowsConflict() refused to bridge a TEAMS
