...
// deliberately puts the installer, the signatures and the provenance bundle
// AHEAD of the raw exe so the matcher's exact-suffix match is proven
// independent of position (the installer ends in `-installer.exe`, a
// signature in `.exe.sig` and the bundle in `.intoto.jsonl`, not
// `windows-amd64.exe`).
