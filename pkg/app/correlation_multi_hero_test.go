...
// heroes_played=[primary, swap]; SCOREBOARD/PERSONAL captured during
...
func TestResolveMatchKey_MultiHero_ScoreboardSwapAdoptsSummary(t *testing.T) {
	// SUMMARY anchored on hero=lucio with heroes_played=[lucio, kiriko];
	// SCOREBOARD captured during the kiriko portion arrives 30 s later.
...
func TestResolveMatchKey_MultiHero_SecondPersonalAdoptsMatch(t *testing.T) {
	// SUMMARY has heroes_played=[heroA, heroB]; SCOREBOARD + PERSONAL 1
...
func TestResolveMatchKey_MultiHero_ReverseOrder_SummaryBridgesToScoreboard(t *testing.T) {
	// SCOREBOARD arrives first (only carries hero=swap, no HeroesPlayed).
	// SUMMARY arrives 30 s later with hero=primary +
	// HeroesPlayed=[primary, swap]. The candidate (SUMMARY) carries
	// HeroesPlayed; the existing row (SCOREBOARD) is the lone hero=swap.
	// Bridge holds because SCOREBOARD's hero is in SUMMARY's
	// HeroesPlayed.
	snap := db.Screenshots{
		Scoreboards: []db.ScoreboardRow{{
