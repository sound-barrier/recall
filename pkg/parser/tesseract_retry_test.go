...
func swapRetrySeams(t *testing.T, fn func(pre image.Image, workDir, name, psm, whitelist string) (string, error)) {
	t.Helper()
...
func TestRunTesseractWithRetry_RecoversTransientFailure(t *testing.T) {
	calls := 0
	swapRetrySeams(t, func(image.Image, string, string, string, string) (string, error) {
		calls++
		if calls < 3 {
	...
	out, err := parser.RunTesseractWithRetry(nil, "", "row", "7", "")
...
func TestRunTesseractWithRetry_GivesUpAfterRetries(t *testing.T) {
	...
	if _, err := parser.RunTesseractWithRetry(nil, "", "row", "7", ""); err == nil {
...
func TestRunTesseractWithRetry_NeverRetriesTimeouts(t *testing.T) {
	...
	if _, err := parser.RunTesseractWithRetry(nil, "", "row", "7", ""); !errors.Is(err, parser.ErrTesseractTimeout) {
...
func TestRunTesseractWithRetry_NoRetryOnSuccess(t *testing.T) {
	...
	if out, err := parser.RunTesseractWithRetry(nil, "", "row", "7", ""); err != nil || out != "ok" {
