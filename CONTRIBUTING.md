...
`frontend/bindings/` is gitignored — `wails3 generate bindings` (or any build) regenerates it from Go sources; nothing imports it. API calls go over HTTP in both modes (the desktop serves the REST mux through the Wails asset server), so adding an endpoint means editing `api/openapi.yaml`, mounting the route, and running `task gen-types` to refresh the generated client under `frontend/src/client/` — devs who only run server mode (devcontainer, CI, remote box) stay in sync without ever booting the GUI. Only the native-dialog methods still dispatch by FQN via `@wailsio/runtime`'s `Call.ByName` (see `frontend/src/api-platform.ts`).
...
- **The gate:** `task check-tool-pins` (also in `task lint` and CI's `lint` job) fails when any of this drifts: a `[tools]` or `[env]` `*_VERSION` value that is not an exact release, a `mise.lock` that disagrees with `mise.toml` or has uncommitted changes, `.node-version` off `[tools] node`, a `go install` literal off its pin, `jdx/mise-action` called outside `setup-mise`, the mise version or hash differing between the four places above, `@latest` or an on-demand package fetch (`npx <pkg>@<version>`, `npx --yes`, `pnpm dlx`, `uvx`, …) in a workflow, Taskfile, hook or script, or a range in `tools/package.json`. Each failure names the file and line.
...
| `gen-types-drift`| `api/openapi.yaml`, `src/client/**` | regenerates the client and fails if the committed output drifts from the spec |
...
- When you **add a field to an existing Go struct** (not a new method), the update is a 2-step follow-up: (1) update the struct + OpenAPI schema, (2) `task gen-types` to refresh the generated client under `frontend/src/client/`. `frontend/bindings/` is a gitignored build artifact regenerated by `wails3 generate bindings` that nothing imports; `api.ts` consumes the generated SDK's types over the single fetch transport, so there's no second file to keep in sync.
...
```sh
task gen-types     # regenerate frontend/src/client
task typecheck     # confirm api.ts still type-checks against the new shape
```

CI runs both and additionally fails if `frontend/src/client` is out of sync with the spec — so commit the regenerated client alongside any spec change.
