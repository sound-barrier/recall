...
|---|---|---|---|
| `build` *(matrix × 4)* | — | One artifact set per matrix target: wails-linux (`.tar.gz` + `.deb`), server-linux (`.tar.gz` + `.deb`), server-windows (`.exe`), server-mac (`.tar.gz`) | Each entry runs on its own ubuntu-latest runner via `docker/build-push-action` against a `Dockerfile.build` target. All four share GHA cache scope `release-build`, so the `go-base` / `server-base` layers materialise once across runs. Packaging logic lives in `scripts/release/package-{wails,server}-{linux,windows,mac}.sh`. |
...
| `build-windows` | — | Windows/amd64 NSIS `installer.exe` | **Native** cross-compile on ubuntu — v3's WebView2 loader is pure Go (`CGO_ENABLED=0`), so no Docker/mingw. `task build-windows` = `wails3 task windows:package` (generate syso → `go build -H windowsgui` → WebView2 bootstrapper → `makensis`); job uses `wails-build-env` + the `nsis` apt package, then `scripts/release/package-wails-windows.sh`. |
