...
- **`go list ./...` includes a stray Go package in `node_modules`** — `frontend/node_modules/flatted/golang/pkg/flatted/` contains real `.go` files treated as part of the `recall` module. Any whole-program Go tool that takes a package list (e.g. `deadcode`) must filter: `go list ./... | grep -v node_modules`. Tools that scan paths directly (e.g. `gosec`) need their native exclude flag instead — `gosec -exclude-dir=frontend ./...`; without it the baseline scan silently reports "0 files inspected" because gosec resolves the import graph and stops on the node_modules conflict.
...
- **`make check-deps` only validates four pins** — Wails CLI, hadolint, lefthook, trivy. Five other pinned tools live outside its scope and need manual checks when bumping: **Spectral CLI** (`@stoplight/spectral-cli@X.Y.Z` appears in `lefthook.yml` `pre-commit.spectral.run`, the `Makefile` `SPECTRAL` variable, AND `.github/workflows/ci.yml` "Lint OpenAPI (Spectral)" step — bump all three in lock-step, same shape as the deadcode known-good filter above), **Swagger UI image** (`Makefile` `SWAGGER_IMAGE`), **Honkit** (`Makefile` `HONKIT_VERSION` + `.github/workflows/pages.yml` env), **typos** (`TYPOS_VERSION` in `initialize.sh` + `.devcontainer/postCreate.sh` + the `crate-ci/typos@vX.Y.Z` pin in `.github/workflows/ci.yml`), and **gosec** (`GOSEC_VERSION` in `initialize.sh` + `.devcontainer/postCreate.sh` + the `go install …@vX.Y.Z` literal in `.github/workflows/ci.yml`'s gosec job). Check with `npm view <pkg> version` for the npm-distributed tools, Docker Hub tag list for `swaggerapi/swagger-ui`, GitHub releases for `crate-ci/typos`, and `go list -m -versions github.com/securego/gosec/v2 | tr ' ' '\n' | tail -5` (or the GitHub releases page) for gosec. A green `make check-deps` does NOT mean everything is current.
...
- **The macOS in-DMG `README.txt` and `docs/install-macos.md` sections 2-3 are paired.** The `.dmg` ships a `README.txt` with the drag-install + Gatekeeper-approval steps (built inline via a heredoc in `.github/workflows/release.yml`'s "Create DMG" step). The same content, formatted as markdown, lives in sections 2-3 of `docs/install-macos.md`. Edit one, edit both — an HTML comment at the top of the synced region in install-macos.md flags this. The web page has a couple of extras the in-DMG copy deliberately skips (the "what you'll see in the DMG" preamble in section 2, the `xattr -d com.apple.quarantine` Terminal alternative in section 3); leave those out of the README.txt to keep the in-DMG note brief.
...
- **Any CI job that loads the root `main` package must first satisfy `//go:embed all:frontend/dist`.** `assets.go` embeds the Vite build output; on a fresh runner `frontend/dist/` doesn't exist and `go build` / `go list` / `gosec` / any other Go-source-loader fails with `pattern all:frontend/dist: no matching files found`. Two fixes: (a) cheap stub when the job only cares about Go source (gosec, deadcode, etc.) — `mkdir -p frontend/dist && touch frontend/dist/index.html`; (b) real build when the job actually needs the assets (e2e, build-server-*) — `cd frontend && npm ci && npm run build`. The gosec job uses (a).
