...
| `pages.yml` | Push to `main` (paths: `api/openapi.yaml`, `docs/api/**`, `pages.yml` itself) + `workflow_dispatch` | Stages `docs/api/index.html` + `api/openapi.yaml` side-by-side and deploys to GitHub Pages via `actions/{configure,upload,deploy}-pages`. Public URL: <https://sound-barrier.github.io/recall/api/>. **One-time setup:** Repo Settings → Pages → Source = "GitHub Actions" (the workflow can't flip this itself; missing it surfaces as `Get Pages site failed`). |
