...
- **Iterating a Playwright e2e spec locally** — `reuseExistingServer: !process.env.CI` in `playwright.config.ts` keeps `recall-server` running across `npx playwright test` runs, but the binary embeds `frontend/dist` at build time. After any `frontend/src/**` or `pkg/**` change, rebuild and kill the stale server before retesting: `cd frontend && npm run build && cd .. && go build -tags serveronly -o /tmp/recall-e2e/recall-server . && lsof -i :7099 | awk 'NR==2 {print $2}' | xargs -r kill`. Symptom of a stale server: locator counts stay at the pre-change value across 14 polling retries even though `page.route()` mocks return the expected data. `make test-e2e` rebuilds for you; the manual sequence is for iteration loops where you want to re-run a single spec without re-running the npm/playwright install steps.

- **`_fetch` in `api.ts` returns `undefined` for HTTP 204.** `r.json()` on a no-body response throws `SyntaxError: Unexpected end of JSON input`, which silently broke `SetMatchVisibility` / `SetMatchAnnotation` / `SetLeaverAnnotation` in server mode until the e2e suite caught it. New endpoints that return `http.StatusNoContent` need their `api.ts` wrappers to discard the return via `.then(() => undefined)`; the `_fetch` helper already handles the empty body.

- **`MatchRecord.Hidden` is a UI filter, not a metrics filter.** The aggregator tags the record so the FilterRail can drop it from the matches list, but `scrapeReader` in `pkg/app/inference.go` does NOT skip on `Hidden` — long-term Grafana trend data sees every competitive match the user played, including ones they later hid from Recall's list. The metrics-layer filter remains `Mode != "competitive"` only. Pinned by `TestApp_ScrapeReader_StillEmitsHiddenMatches`. If we ever decide hidden should also drop from Prometheus, the filter belongs in `scrapeReader` (one decision point for both surfaces).
