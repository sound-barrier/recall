...
- **Match keys are URL-safe** — `match_key` is `match-<ISO-timestamp>` (with `-` separating date AND time components, e.g. `match-2026-05-10T22-21-11`), `unmatched-<filename>`, or `ambiguous-<filename>`. No colons inside the key, so pasting it raw into a URL path "just works" (the embedded filename for unmatched/ambiguous still needs the usual encoding for spaces). A one-time SQL migration in `db.NewSQLStore::migrateMatchKeysColonToDash` rewrites any legacy `match:` / `unmatched:` / `ambiguous:` row on startup. The frontend's `matchTime()` helper rewrites the `-` time separators back to `:` for display purposes; `data.finished_at` is unaffected (still `HH:MM:SS`).

- **`MatchRecord.Hidden` is a UI filter, not a metrics filter.** Aggregator tags it so the MatchesView can honor a "show hidden" toggle; `scrapeReader` in `pkg/app/inference.go` does NOT skip on `Hidden` — Grafana trends see every competitive match including hidden ones. Metrics-layer filter stays `Mode != "competitive"` only. Pinned by `TestApp_ScrapeReader_StillEmitsHiddenMatches`. If we ever filter hidden from Prometheus too, do it in `scrapeReader` (one decision point).

- **Vue `<style scoped>` miscompiles `:global(X) .y { … }` partial form.** The compiler strips `.y` and emits a bare `X { … }` rule — e.g. `:global([data-theme="light"]) .link-btn { color: ... }` becomes `[data-theme="light"] { color: ... }` matching `<html>` directly. If the body sets `opacity`/`background`/anything beating `style.css`'s `html[data-theme="light"]` specificity (0,0,1,1), the SFC pollutes the whole page once mounted (scoped tags persist in `<head>` after unmount). Put cross-theme overrides in `app.css` scoped under a parent id (`[data-theme="light"] #panel-settings .x`), not `<style scoped>`. Verify: `cd frontend && npm run build && grep -c "^\[data-theme=light\]{" dist/assets/*.css` must stay 0. Latent in `ParseProgressPanel.vue` / `ParseStatusBar.vue` but harmless (color/background only).
...
- **Breaking changes are fine — just declare them.** Pre-1.0, the project is not contractually stable; any layer (HTTP `/api/v1/...` shape, Wails IPC method signature, SQLite column/table layout, `settings.json` schema, exported Go API, on-disk export format) can change. Conventional Commits is the declaration channel — release-please reads it and cuts the right SemVer bump. Two forms; pick by where the breaking fact is easiest to read:
  - `feat!: rename SetLeaverAnnotation arg` — exclamation after the type marks the whole commit breaking. Subject already tells the story.
  - `BREAKING CHANGE: <one line>` footer — use when the breaking part is one detail inside a larger commit body (the body explains the why; the footer is the structured marker release-please grep-matches).
  Both produce a **minor** bump pre-1.0 (release-please's `bumpMinorPreMajor: true`) and a **major** bump after 1.0. A `BREAKING CHANGE` line MUST appear somewhere release-please can find it (subject or footer) — otherwise the change ships as a silent patch. Recipes: SQL schema drop a column → `feat!:` + migration note in body; Wails method rename → `feat!:` + new IPC name; HTTP route move → `feat!:` + old/new path in body and OpenAPI spec edit. Don't add backwards-compat shims to "soften the landing" — that's [What to avoid](#what-to-avoid) — declare and break clean.
...
- **Theme set is Day / Dark / Night / High contrast.** Storage key `recall.theme`; legacy values (`light`, `ow-light`, `ow-dark`) get silently migrated by `useTheme.parseTheme` so user prefs survive. `dark` intentionally is NOT migrated — the string is reused by the new OW-gray palette. Day = OW White ground, Dark = OW gray structural plates over a `#2a2a2a` page bg (the literal `#4A4A4A` brand-gray would cap accent contrast under WCAG AA), Night = the editorial `:root` palette (no scoped override; cascade lands there), HC = pure-black / pure-white / boosted-gold tournament-booth mode.

- **04 Analysis tab is dev-build-only.** `appVersion.endsWith('-dev')` is the gate (`isDevBuild` in App.vue). `visibleTabs` filters `TAB_ORDER` and is passed as the optional 3rd arg to `useTabKeyboardNav` so ←/→ wrap-around stays inside rendered tabs. The `MatchesDashboardSketch` panel adds `&& isDevBuild` as defence-in-depth. Don't re-enable on release without finishing the underlying dashboard wiring.

- **Update check is user-pulled, not auto-fired on mount.** The masthead's `.ver-block` defaults to a "Check for updates" button (`.ver-btn-check`); clicking fires `GET /api/v1/system/update` and branches into the existing dev_build / available / up-to-date result states. The earlier shape made an unsolicited GitHub releases call on every boot — regressed silently when the masthead got rewired. Don't re-add the auto-fire without re-evaluating the metered-connection / consent argument.
