...
| `codeql.yml` | Push or PR to `main` + weekly cron | GitHub CodeQL static analysis. Matrix covers `go`, `javascript-typescript`, and `actions` (the last parses `.github/workflows/*.yml` + composite actions for script-injection sinks, missing `persist-credentials`, untrusted-input → env flows, etc.). Findings appear in the Security tab; surfaced by the CodeQL badge on the README. Provides JS/TS SAST coverage so no separate Semgrep CI job is needed — Semgrep runs only locally via lefthook. |
