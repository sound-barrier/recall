...
: "${MAX_TOTAL_JS_BYTES:=394000}"   # 394 KB — every JS chunk combined (PR E added density picker; PR F added Unknown section nav; PR 3 added candidate-thumb buttons; queue-type chooser + Queue filter chip; play-mode chooser + Play mode filter chip; Quickplay-vs-Competitive + Winrate-by-play-mode widgets + playModeBreakdown dossier helper; ambiguous candidate-picker side-by-side preview pane + auto-open source on row click)
: "${MAX_TOTAL_CSS_BYTES:=223000}"  # 223 KB — every CSS chunk combined (queue chooser + play-mode chooser radiogroup styles; candidate-picker grid + preview-pane styles)
