#!/usr/bin/env bash
# Walk the OWMetrics → Prometheus pipeline layer by layer and report which
...
if metrics_body=$(curl -fsS --max-time 3 "$METRICS_URL" 2>/dev/null); then
  ...
  ow_lines=$(printf '%s\n' "$metrics_body" | grep -c '^owmetrics_' || true)
  if (( ow_lines > 0 )); then
    pass "$ow_lines owmetrics_* sample line(s) in the response"
  else
    fail "endpoint up but no owmetrics_* samples — collector isn't producing data"
...
if state=$(podman inspect -f '{{.State.Status}} {{.RestartCount}}' owmetrics-prometheus 2>/dev/null); then
  status="${state% *}"
  restarts="${state#* }"
  if [[ "$status" == "running" ]] && (( restarts < 3 )); then
    pass "owmetrics-prometheus container running (restart count: $restarts)"
  elif [[ "$status" == "running" ]]; then
    fail "container is running but has restarted $restarts times — likely crash-looping"
    note "see: podman logs --tail 20 owmetrics-prometheus"
  else
    fail "container state: $status"
  fi
else
  fail "owmetrics-prometheus container not found — run ./scripts/stack-up.sh"
fi

# ── Layer 4: Prometheus scrape state ───────────────────────────────────────
echo
echo "[4] Prometheus scrape of owmetrics target"
if up_resp=$(curl -fsS --max-time 3 "$PROM_URL/api/v1/query?query=up%7Bjob%3D%22owmetrics%22%7D" 2>/dev/null); then
  pass "Prometheus API reachable"
  state=$(printf '%s' "$up_resp" | jq -r '.data.result[0].value[1] // empty' 2>/dev/null)
  case "$state" in
    1) pass "owmetrics target is UP (Prometheus can reach Wails app)" ;;
    0) fail "owmetrics target is DOWN — Prometheus can't reach $METRICS_URL"
       note "from inside the VM, host.docker.internal:$METRICS_PORT must be reachable"
       note "see: curl $PROM_URL/api/v1/targets" ;;
    *) fail "no 'up' sample for owmetrics yet — Prometheus may not have scraped (wait ~30s)" ;;
  esac
else
...
# OWMetrics samples are timestamped at the match's `finished_at` time, which
...
query='count(count_over_time(owmetrics_match_eliminations%5B365d%5D))'
if count_resp=$(curl -fsS --max-time 3 "$PROM_URL/api/v1/query?query=$query" 2>/dev/null); then
  ...
  if [[ "$count" =~ ^[0-9]+$ ]] && (( count > 0 )); then
    ...
    if [[ "${rows:-}" =~ ^[0-9]+$ ]] && (( count == rows )); then
      ...
      note "see: $PROM_URL/graph?g0.expr=count_over_time(owmetrics_match_eliminations%5B365d%5D)"
    fi
  else
    ...
    note "or query manually: $PROM_URL/graph?g0.expr=owmetrics_match_eliminations"
  fi
fi
