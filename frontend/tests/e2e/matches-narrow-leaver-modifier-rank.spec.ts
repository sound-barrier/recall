...
const match = (i: number, o: { result: string; rank: string; leavers?: string[]; modifiers: string[] }) => ({
  match_key: `m${i}`,
  ...
  ...(o.leavers ? { annotation: { leavers: o.leavers, throwers: [], members: [] } } : {}),
...
const CORPUS = [
  ...
  match(3, { result: 'defeat', rank: 'diamond', leavers: ['enemy'], modifiers: ['reversal', 'defeat'] }),
  match(4, { result: 'defeat', rank: 'diamond', leavers: ['enemy'], modifiers: ['reversal', 'defeat'] }),
  match(5, { result: 'victory', rank: 'platinum', leavers: ['team'], modifiers: ['calibration', 'victory'] }),
]
