// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT
import {app} from '../models';
import {http} from '../models';

export function GetMatchResults():Promise<Array<app.MatchRecord>>;

export function GetPrometheusEnabled():Promise<boolean>;

export function GetScreenshotsDir():Promise<string>;

export function GetTesseractStatus():Promise<app.TesseractStatus>;

export function GetWatchEnabled():Promise<boolean>;

export function ParseScreenshots():Promise<void>;

export function PickScreenshotsDir():Promise<string>;

export function PickTesseractBinary():Promise<app.TesseractStatus>;

export function ResetTesseractPath():Promise<app.TesseractStatus>;

export function ScreenshotHandler():Promise<http.Handler>;

export function SetPrometheusEnabled(arg1:boolean):Promise<void>;

export function SetScreenshotsDir(arg1:string):Promise<void>;

export function SetTesseractPath(arg1:string):Promise<app.TesseractStatus>;

export function SetWatchEnabled(arg1:boolean):Promise<void>;
