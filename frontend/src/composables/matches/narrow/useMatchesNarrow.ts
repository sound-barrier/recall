...
import { TIER_ORDER, FILTERABLE_MODIFIERS, RESULT_MODIFIERS } from '@/match/trends/match-trends-helpers'
...
export { createMatchesNarrowState } from '@/composables/matches/narrow/matchesNarrow.state'
...
export function useMatchesNarrow(
  ...
  // The Hero Pool band pushes its In-pool / Out-of-pool selection here — a
  // snapshot of the current mode's pool keys + side. Passing null (or re-picking
  // the active side) clears it.
  function setPoolFilter(filter: PoolFilter | null) {
