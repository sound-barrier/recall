...
describe('MostPlayedHeroWidget', () => {
  it('renders the top-ranked hero name', () => {
    const w = mount(MostPlayedHeroWidget, {
      props: {
        topHeroes: [hero('lucio', 60, 55), hero('mercy', 40, 50)],
  ...
  it('renders em-dash when topHeroes is empty', () => {
    const w = mount(MostPlayedHeroWidget, {
      props: { topHeroes: [], mostPlayedHero: null },
    })
