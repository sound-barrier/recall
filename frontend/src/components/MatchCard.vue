...
/* `.match-header` + title-row + tag-row + chip styles + incomplete
   badge + leaver-mark + note-mark + compact-stats moved to
   MatchCardHeader.vue. `:where(button.hero-name, button.source-type-chip,
   button.slot-chip)` chrome reset stays here for the .match-expanded
   buttons. */
:where(
  button.hero-name,
...
/* ─── Compact density: only the body padding stays here ─── */

/* Tightens the collapsed-card outer body by ~50% vertically. The
   header-specific compact overrides moved to MatchCardHeader.vue
...
/* `.leaver-mark` (collapsed-view pill) moved to MatchCardHeader.vue
   along with the markup. The chooser below stays here because it
   lives inside .match-expanded. */
...
/* `.note-mark` (collapsed-view pill) moved to MatchCardHeader.vue. */
