...
import { refetchSelfReviews, upsertSelfReview, useSelfReviewsQuery } from '@/queries/selfReview'
...
export const useSelfReviewStore = defineStore('selfReview', () => {
  ...
  // Hydrate the room from a sitting the server handed back — the list's copy
  // when it has one, a GET otherwise. Any queued saves for a previous sitting
  // are flushed first, so switching sittings never drops a keystroke.
  async function openSitting(reviewID: string): Promise<void> {
    await flushSaves()
    discardSaves()
    const sitting = reviews.value.find((r) => r.review_id === reviewID) ?? await GetSelfReview(reviewID)
  ...
  // land there through the cache once the sitting is re-read. Cheap and
  // honest: one GET of the list on close, not one per keystroke.
  ...
  /** Finish: flush, stamp the sitting done (and its matches reviewed by self), back to the shelf. */
  ...
  // block is gone from the match, so the desk must not keep showing it.
  ...
  return {
    ...
    openSitting, createFromKeys, selectKey,
