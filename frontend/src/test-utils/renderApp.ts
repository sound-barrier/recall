...
// The mocked '@/api' the current render is wired against. Throws if called
// before renderApp() so a misuse fails loudly rather than reading a stale mock.
export function mockedApi(): Record<string, ReturnType<typeof vi.fn>> {
  if (!lastMockedApi) throw new Error('mockedApi() called before renderApp()')
...
// fireBackendEvent reaches into the captured EventsOn handler map and
// invokes the named handler synchronously. Tests use this to simulate a
// Wails EventsEmit / SSE broadcast hitting the App in mid-flight. (Named
// fireBackendEvent — not fireEvent — so it can't shadow Testing
// Library's own fireEvent in migrated tests.)
//
// Note: the mocked GetMatchResults always returns whatever `records` the
// test seeded at render time, so to verify a record-list change after a
// fired event you'll typically want to re-mock the API or assert on a
// non-record-count side-effect (animations, pulses, etc.).
export function fireBackendEvent(name: string, data: unknown = undefined): boolean {
...
// renderApp installs the api mock via the api-client seam (no module-mock
// dance), dynamically imports App.vue, and flushes so the onMounted load()
// chain settles before tests assert on the rendered DOM.
export async function renderApp(overrides: MountOverrides = {}): Promise<RenderResult> {
  // Fresh module graph each render so module-level composable state
  ...
  // would need to delete this seed before rendering.
  ...
  // want to exercise the modal can delete this key before rendering.
  localStorage.setItem('recall.firstRunAccountNamed', 'true')

  // Install the api mock on THIS render's api-client instance (imported
