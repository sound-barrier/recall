...
- **Skip allow-list** — every `t.Skip` in `pkg/` must appear in
  `scripts/test-skips-allow.txt` with a one-line "why" comment.
...
- **Third-party GitHub Actions are SHA-pinned with a `# vX.Y.Z` comment** —
  `scripts/check-action-pins.sh` enforces from `make lint-actions`, lefthook
